...
class CriarContaController: NSObject {
	...
	func createUserFirebase(email: String?, password: String?, completion: @escaping(_ success: Bool, _ errorCode: Int?) -> Void) {
		OnboardWorker().createUserFirebase(email: email, password: password) { (success, errorCode) in
	...
	func createUser(nameDisplay: String?, nameFull: String?, email: String?, cpf: Int64, dateBrith: Date?) {
        OnboardWorker().createUser(nameDisplay: nameDisplay, nameFull: nameFull, email: email, cpf: cpf, dateBrith: dateBrith)
