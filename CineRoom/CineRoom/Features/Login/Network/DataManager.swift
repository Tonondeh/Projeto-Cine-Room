...
class DataManager {
    ...
    func loadUserData(email: String,completion: @escaping(_ success:String?) ->Void){
        ...
        //        let predicate = NSPredicate(format: "email CONTAINS %@", "lucas.munho1994@gmail.com")
        //        request.predicate = predicate
        var arrayUser:[UserData]?
        do {
            //           let userData =  try self.managedContext.fetch(request)
             arrayUser =  try self.managedContext.fetch(UserData.fetchRequest())
//            let arrayUser:[UserData] = userData
            if let _userData = arrayUser?[0]{
                completion(_userData.nameDisplay)
